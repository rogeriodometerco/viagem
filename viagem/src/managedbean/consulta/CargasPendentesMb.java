...
public class CargasPendentesMb implements Serializable {
	...
	public void listar() {
		...
		for (int i = 0; i < 40; i++) {
			...
			item.setStatus(Arrays.asList("Pendente", "Pendente", "No local - coleta", "Carregado", "No local - entrega", "Carregado", "Trânsito", "Descarregado").get(i%8));
	...
	private void inicializarMatriz() {
		...
		Coluna colunaNoLocal = new Coluna();
		Coluna colunaCarregado = new Coluna();
		Coluna colunaTransito = new Coluna();
		Coluna colunaNoLocal2 = new Coluna();
		Coluna colunaDescarregado = new Coluna();
		...
		colunas.add(colunaNoLocal);
		colunas.add(colunaCarregado);
		colunas.add(colunaTransito);
		colunas.add(colunaNoLocal2);
		colunas.add(colunaDescarregado);
		...
		colunaNoLocal.setTitulo("No local - coleta");
		colunaCarregado.setTitulo("Carregado");
		colunaTransito.setTitulo("Trânsito");
		colunaNoLocal2.setTitulo("No local - entrega");
		colunaDescarregado.setTitulo("Descarregado");
		...
		for (Item item: lista) {
			...
			Celula noLocal = new CelulaNumero();
			Celula carregado = new CelulaNumero();
			Celula transito = new CelulaNumero();
			Celula noLocal2 = new CelulaNumero();
			Celula descarregado = new CelulaNumero();
			...
			noLocal.setDado(item.getStatus().equals("No local - coleta") ? 1 : 0);
			carregado.setDado(item.getStatus().equals("Carregado") ? 1 : 0);
			transito.setDado(item.getStatus().equals("Trânsito") ? 1 : 0);
			noLocal2.setDado(item.getStatus().equals("No local - entrega") ? 1 : 0);
			descarregado.setDado(item.getStatus().equals("Descarregado") ? 1 : 0);
			...
			celulas.add(noLocal2);
			celulas.add(descarregado);

			matriz.criarLinha(celulas);
		}

		colunaPendente.setAcumulada(true);
		colunaNoLocal.setAcumulada(true);
		colunaCarregado.setAcumulada(true);
		colunaTransito.setAcumulada(true);
		colunaNoLocal2.setAcumulada(true);
		colunaDescarregado.setAcumulada(true);
	...
	public void agrupar() {
		porDestinoProduto = matriz.agrupar();
	}
