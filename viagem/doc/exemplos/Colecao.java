...
public class Colecao {
	...
	public Colecao() {
		...
		List<FiltroPessoa> filtrosPessoa = new ArrayList<FiltroPessoa>();
		filtrosPessoa.add(new FiltroPessoa("T"));
		filtrosPessoa.add(new FiltroPessoa("g"));
		filtrosPessoa.add(new FiltroPessoa("o"));

		filtros = new Object[2];
		filtros[0] = filtrosPessoa;
		filtros[1] = new ArrayList<FiltroData>(Arrays.asList(new FiltroData(calendar.getTime(), new Date())));
	...
	private List<Filtro> montarFiltros(int col) {
		Set<Object> opcoes = (Set<Object>)opcoesParaFiltro[col];
		for (Object o: opcoes) {
			Filtro f = new FiltroPessoa("rogerio");
			//f.adicionarOpcao(o);
		}
		return null;
	}
		...
		for (int lin = 0; lin < dados.length; lin++) {
			...
			for (int col = 0; col < dados[lin].length; col++) {
				for (int i = 0; i < filtros.length; i++) {
					...
					for (Filtro f: (List<Filtro>)filtros[col]) {
	...
	public void agrupar() {
		...
		for (int lin = 0; lin < dados.length; lin++) {
			...
			FiltroPessoa filtro = new FiltroPessoa(((Pessoa)linha[0]).getNome());
